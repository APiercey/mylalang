...
fn tokenize_opening_hash_bracket(input: &str) -> Token {
    return tokenize_character(Kinds::OpeningHashBracket, '{', input);
}

fn tokenize_closing_hash_bracket(input: &str) -> Token {
    return tokenize_character(Kinds::ClosingHashBracket, '}', input);
}
...
pub fn tokenize(full_prg: &str) -> Vec<Token> {
    ...
    let functions: Vec<&dyn Fn(&str) -> Token> = vec![
        ...
        &tokenize_opening_hash_bracket,
        &tokenize_closing_hash_bracket,
        &tokenize_string,
        &tokenize_number,
        &tokenize_word,
    ];
