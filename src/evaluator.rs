...
pub fn evaluate(env: Env, root_ast: Types) -> Types {
    let return_type: Types;
    let mut ast = root_ast.clone();

    'tco: loop {
        return_type = match ast {
            Types::List(ref l) => {
                ...
                match action {
                    ...
                    Types::Word(word) => match word.as_str() {
                        ...
                        "let" => {
                            ...
                            if l[2..].len() > 2 {
                                for t in l[2..(l.len())].iter() {
                                    evaluate(env.clone(), t.clone());
                                }
                            }

                            ast = match l[2..].last() {
                                Some(t) => t.clone(),
                                None => panic!("I don't think is is possible"),
                            };
                        ...
                        "and" => {
                            let result =
                                l[1..]
                                    .to_vec()
                                    .into_iter()
                                    .fold(Types::Bool(true), |state, t| match evaluate_bool_value(
                                        state.clone(),
                                    ) {
